...
public class StreamConfigBuilderTests
{
    ...
    public void AddSource_WithExternal_AddsSourceWithExternal()
    {
        ...
        Assert.Equal("$JS.hub.API", config.Sources[0].External!.ApiPrefix);
        Assert.Equal("deliver.hub", config.Sources[0].External.DeliverPrefix);
    }
    ...
    public void WithAllowRollup_SetsAllowRollup()
    {
        ...
        // Assert
        Assert.True(config.AllowRollup);
    }
