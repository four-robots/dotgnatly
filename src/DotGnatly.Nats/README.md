...
This is the NATS-specific implementation library for DotGnatly, providing concrete implementations of the DotGnatly.Core interfaces using NATS as the underlying message broker.

## Purpose

DotGnatly.Nats provides:

- **Bindings**: P/Invoke bindings and native interop code for the NATS C library
- **Implementation**: Concrete implementations of DotGnatly.Core interfaces that use NATS for message transport

## Dependencies

- **DotGnatly.Core**: Reference to the core abstractions and interfaces
...
Configuration is managed through the DotGnatly.Core configuration models. NATS-specific options include:
- Server URLs
- Connection timeouts
